...
class ArgParserBy<ITEM>(private val prog: String, vararg val items: Arg<ITEM>) {
  ...
  inner class ByFlag(private val p: Arg<*>) { init { flags.add(p) }
    operator fun <M> getValue(_m:M, _p:KProperty<*>): Boolean = p.secondName in res.flags
    operator fun <M> setValue(_m:M, _p:KProperty<*>, v: Boolean) { res.flags = res.flags.replace(p.secondName, "") }
  }
  ...
  inner class ByArg<T>(p: Arg<T>): ByArgParse<T>(p) {
    ...
    operator fun <M> getValue(_m:M, _p:KProperty<*>): T = @Suppress("unchecked_cast") (sto.get() as T)
    operator fun <M> setValue(_m:M, _p:KProperty<*>, v: T) { sto = OneOrMore<Any>(v) }
  ...
  inner class ByArgConvert<T, R>(p: Arg<T>, private val transform: (OneOrMore<T>) -> R): ByArgParse<T>(p) {
    operator fun <M> getValue(_m:M, _p: KProperty<*>): R = @Suppress("unchecked_cast") (sto as OneOrMore<T>).let(transform)
  }
  inner class ByArgRepeat<T>(p: Arg<T>): ByArgParse<T>(p) {
    operator fun <M> getValue(_m:M, _p:KProperty<*>): List<T> = @Suppress("unchecked_cast") (sto.toList() as List<T>)
    operator fun <M> setValue(_m:M, _p:KProperty<*>, v: List<T>) { sto = OneOrMore<Any>().apply { v.forEach { add(it as Any) } } }
  }
